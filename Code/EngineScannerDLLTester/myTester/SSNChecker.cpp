	...
	DLL_EXPORT int ScanSSN(char s[], int length, int *count, struct SSNPatternCounter *patternCount)
	{
		...
		*count = 0;
		...
		for(;next != end; next++)
		{
			...
			for(i = 0; i < (int) NineDigit.size(); ++i)
			{
				if(NineDigit[i] == '-')
				{
...
int main(void)
{
	...
	{
		int length, count;
		SSNPatternCounter patternCount;
		fin.seekg(0, std::ios::end);    // go to the end
		length = (int) fin.tellg();           // report location (this is the lenght)
		fin.seekg(0, std::ios::beg);	// rewind to the beginning
		char *str = new char[length];	// char array the exact size of the file
		fin.read(str, length);			// read the whole file into the string
		fin.close();
		str[length] = '\0';				// make sure it's NULL terminated

		int testPri = ScanSSN(str, length, &count, &patternCount);
		cout << "returned priority: " << testPri << endl;
		cout << "returned count: " << count << endl;
		cout << "returned pattern: {" << patternCount.d9 << ", " << patternCount.d324 << "}" << endl;
		cin >> *str; // keep the output showing
		delete str;
	}
}
