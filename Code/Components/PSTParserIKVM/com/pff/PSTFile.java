...
public class PSTFile {
	...
	public String processFolder(PSTFolder folder)
			throws PSTException, java.io.IOException
	{
		processFolderRec(folder);
		return text.toString();
	}
		public void processFolderRec(PSTFolder folder)
			throws PSTException, java.io.IOException
	{
		depth++;
		...
		if (folder.getContentCount() > 0) {
			...
			while (email != null) {
				...
				//System.out.println("\tMessage Body: " + email.getBody());
				text.append(email.getSubject() + "\n" + email.getBody());
